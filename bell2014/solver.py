...
class IntrinsicSolver(object):
    ...
    def stage1_optimize_r(self):
        ...
        # TODO: modify the input interface of nrank (and use a better name?)
        if self.params.learned_prior_weight > 0:
            nrank = self.W_ns.shape[0]
        else:
            nrank = 0

        # use a Python wrapper around the code from [Krahenbuhl et al 2013]
        densecrf = DenseCRF(npixels, nlabels, nrank)
        ...
        if self.params.learned_prior_weight:
            # pairwise comparison costs
            same_compatibility = np.zeros((nlabels, nlabels))
            darker_compatibility = np.zeros((nlabels, nlabels))
            brighter_compatibility = np.zeros((nlabels, nlabels))
            log_intensities = np.log(self.decomposition.intensities + 1e-10)
            for l1 in range(nlabels):
                for l2 in range(nlabels):
                    v1 = log_intensities[l1]
                    v2 = log_intensities[l2]
            ...
            densecrf.add_comparison_energy_nystrom(
                ...
                same_compatibility.astype(np.float32))
            ...
            densecrf.add_comparison_energy_nystrom(
                ...
                brighter_compatibility.astype(np.float32))
    ...
    def split_label_clusters(self, connectivity=2):
